import { IDrawingContext, Position, Size } from "./common.js";
...
function measureLines(lines: string[]): Size {
    const width = lines.reduce((x, y) => Math.max(x, y.length), 0);

    return { width, height: lines.length };
}

function drawLineInPixel(from: Position, to: Position) {
...
export class AsciiTextDrawingContext implements IDrawingContext {
    private lines: number[][] = [];

    public drawLines(...positions: Position[]): void {
    ...
    public drawRectangle(position: Position, size: Size): void {
        this.drawLines(
            position,
    ...
    public drawText(text: string, position: Position): void {
